...
>
	<IconAlertTriangle
		size={20}
		stroke={2}
		class="transition-colors text-primary-600 group-hover:text-amber-600"
	/>
</button>
...
>
	<div class="p-4 sm:p-6 space-y-4">
		...
		<!-- Story Title -->
		<div class="p-3 bg-primary-25 dark:bg-graphite-900 rounded-lg">
			<p class="text-sm text-primary-600">
				{s('article.reportModal.reportingFor')}
			</p>
			<p class="font-medium text-primary line-clamp-2">
				{title}
			</p>
		</div>
		...
		<div>
			<fieldset>
				<legend class="block text-sm font-medium text-primary-700 mb-2">
				...
				<div class="space-y-2">
					...
					<label class="flex items-center">
						<input
							...
							class="me-2 text-amber-600 focus:ring-amber-500"
						/>
						<span class="text-sm text-primary-700">{s('article.reportModal.issueType.other')}</span>
					</label>
				</div>
			</fieldset>
		</div>
		...
		<!-- Description Input -->
		<div>
			<label for="report-description" class="block text-sm font-medium text-primary-700 mb-2">
			...
			<textarea
				...
				class="w-full h-[180px] px-3 py-2 border border-primary-200 rounded-lg bg-input-bg text-primary placeholder-primary-600 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed resize-none overflow-y-auto"
		...
		<fieldset>
			<div class="mb-2">
				<legend class="block text-sm font-medium text-primary-700">
				...
				{#each sourceUrls as sourceUrl, index}
					<div class="flex gap-2">
						<input
							...
							class="flex-1 px-3 py-2 border border-primary-200 rounded-lg bg-input-bg text-primary placeholder-primary-600 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
						/>
						{#if sourceUrls.length > 1}
							<button
								...
								disabled={isSubmitting || isSuccess}
								class="p-2 text-primary-600 hover:text-red-600 disabled:opacity-50 disabled:cursor-not-allowed"
		...
		<div class="flex justify-end gap-3 pt-2">
			<button
				onclick={closeModal}
				disabled={isSubmitting || isSuccess}
				class="px-4 py-2 text-sm font-medium text-primary-700 bg-modal-bg border border-primary-200 rounded-lg hover:bg-primary-25 focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:opacity-50 disabled:cursor-not-allowed"
			>
				{s('article.reportModal.cancel')}
			</button>
			<button
				onclick={handleSubmit}
				disabled={isSubmitting || !description.trim() || !issueType || isSuccess}
