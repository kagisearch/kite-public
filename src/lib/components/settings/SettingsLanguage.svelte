...
<div class="space-y-8">
		...
		<div class="space-y-4">
			...
			<div class="ps-2 space-y-6">
				...
				<div>
					...
					<p class="mb-3 text-xs text-primary-600">
					...
					<div class="flex items-center gap-2">
						{#each globalReadingLevels as level}
							<button
								...
								class="px-4 py-2 text-sm rounded-lg transition-colors {readingLevelSettings.global ===
								level.value
									? 'bg-purple-100 dark:bg-purple-900 text-accent-links font-medium'
				...
				<div class="pt-4 border-t border-primary-100">
					...
					<p class="mb-4 text-xs text-primary-600">
					...
					{#if enabledCategories.length === 0}
						<p class="text-sm text-primary-600 italic">
							{s('settings.readingLevel.noCategories') ||
								'No categories enabled. Enable categories in the Categories tab.'}
						</p>
					{:else}
						<div class="space-y-2">
							{#each enabledCategories as category}
								...
								<div
									class="flex items-center justify-between py-2 border-b border-primary-50 last:border-b-0"
								>
									<div class="flex items-center gap-2">
										<span class="text-sm font-medium text-primary-700 capitalize">
											{category.name}
										</span>
										{#if effectiveLevel && effectiveLevel !== 'normal'}
											<span
												class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900 text-accent-links"
											>
												<IconSparkles size={12} class="me-1" />
												{getLevelLabel(effectiveLevel)}
												{#if currentOverride === 'use-global'}
													<span class="ms-1 opacity-70"
														>({s('settings.readingLevel.fromGlobal') || 'global'})</span
													>
												{/if}
											</span>
										{/if}
									...
									<div class="flex items-center gap-1">
										{#each categoryReadingLevels as level}
											<button
												...
												class="px-2.5 py-1 text-xs rounded-md transition-colors {currentOverride ===
												level.value
													? 'bg-purple-100 dark:bg-purple-900 text-accent-links'
													: 'bg-primary-50 text-primary-600 hover:bg-primary-100'}"
												title={level.description}
											>
												{s(level.labelKey) || level.fallback}
				...
				<!-- Info Section -->
				<div class="pt-4 border-t border-primary-100">
					<div class="flex items-start gap-3 text-sm text-primary-600">
						...
						<div class="space-y-2">
							...
							<p>
								<strong>{s('story.simplify.simple') || 'Simple'}</strong>: {s(
									'settings.readingLevel.info.simpleDescription',
								) || 'Clear language at an intermediate level (B1). Good for general readers.'}
							</p>
							<p>
								<strong>{s('story.simplify.verySimple') || 'Very Simple'}</strong>: {s(
									'settings.readingLevel.info.verySimpleDescription',
								) || 'Basic vocabulary and short sentences (A2). Ideal for language learners.'}
							</p>
						</div>
					</div>
				</div>
	...
	{:else}
		...
		<div class="space-y-4">
			...
			<div class="ps-2">
				<div class="rounded-lg border border-primary-100 bg-primary-25 p-4 dark:bg-graphite-850/50">
					<p class="text-sm text-primary-600">
						{s('settings.readingLevel.subscriberOnly') ||
							'Reading level simplification is available for Kagi subscribers. Upgrade to automatically simplify articles to your preferred reading level.'}
					</p>
					<a
						href="https://kagi.com/settings?p=billing"
						target="_blank"
						rel="noopener noreferrer"
						class="mt-3 inline-block text-sm text-accent-links hover:opacity-80"
					>
