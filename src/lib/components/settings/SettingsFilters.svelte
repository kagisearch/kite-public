...
import { getContentFilterPresetKeywords } from '$lib/data/knPrefs';
...
function getPresetKeywords(preset: any): string[] {
	return getContentFilterPresetKeywords(preset, dataLanguage.current);
}
...
<div class="space-y-6">
	...
	<div>
		<div class="flex items-center justify-between mb-1">
			...
			<a
				...
				target="_blank"
				class="text-sm text-accent-links"
			>
				{s('settings.contentFilter.contribute') || 'Add more filters'}
			</a>
		</div>
		<p class="mt-0.5 mb-3 text-sm text-primary-600">
			...
			{#each localizedPresets as preset}
				<button
					...
					class="flex items-center justify-between w-full px-3 py-2 text-sm rounded-md border transition-colors
						...
						? 'bg-purple-50 border-purple-300 text-accent-links dark:bg-purple-900/20 dark:border-purple-700'
					...
					<span class="flex items-center gap-2">
						...
						{#if preset.tooltip}
							<Tooltip text={preset.tooltip} position="top">
								<IconInfoCircle size={14} class="text-primary-400" />
							</Tooltip>
						{/if}
					</span>
	...
	<div>
		<label for="keyword-input" class="text-base font-medium text-primary">
			{s('settings.contentFilter.keywords.label') || 'Custom Keywords'}
		</label>
		<p class="mt-0.5 mb-2 text-sm text-primary-600">
		...
		<div class="flex gap-2">
			<input
				...
				class="flex-1 rounded-md border border-primary-200 bg-input-bg px-3 py-2 text-primary placeholder:text-primary-400 focus:border-purple-600 focus:bg-input-bg focus:outline-none"
			/>
			<button
				onclick={addKeyword}
				disabled={!newKeyword.trim()}
				class="rounded-md bg-purple-600 px-4 py-2 text-white hover:bg-purple-700 disabled:bg-primary-200 disabled:cursor-not-allowed dark:bg-purple-600 dark:hover:bg-purple-700"
			>
				<IconPlus size={20} />
			</button>
		</div>
	...
	{#if contentFilter.keywords.length > 0}
		<div>
			<h4 class="text-sm font-medium text-primary-700 mb-2">
						...
						{#each customKeywords as keyword}
							...
							>
								...
								<button
									...
									class="ms-1 text-primary-600 hover:text-primary-700"
		...
		<div class="space-y-2">
			<label class="flex items-start">
				<input
					...
					class="mt-0.5 h-4 w-4 text-accent-links focus:ring-focus-ring dark:focus:ring-focus-ring"
				/>
				<div class="ms-3">
					<span class="block text-sm font-medium text-primary-700">
	...
	<div class="space-y-2">
		<Select
			...
			onChange={(value: string) => contentFilter.setFilterScope(value as FilterScope)}
		/>
	...
	<div class="flex items-center justify-between">
		<div>
			<span id="label-show-count" class="text-base font-medium text-primary">
		...
		<button
			...
			class="focus-visible-ring relative inline-flex h-6 w-11 items-center rounded-full transition-colors {contentFilter.showFilteredCount
	...
	<div class="pt-4 border-t border-primary-100">
		<h4 class="text-sm font-medium text-primary-700 mb-3">
		...
		<div class="flex gap-3 relative">
			<button
				...
				class="flex items-center gap-2 px-4 py-2 text-sm bg-primary-100 hover:bg-primary-100 rounded-md text-primary-700"
			>
			...
			{#if showImportConfirm}
				...
				>
					<div class="bg-modal-bg rounded-lg shadow-lg border border-primary-100 p-4 w-80">
						...
						{/if}
						<p class="text-xs text-primary-600 mb-3">
						...
						<div class="flex gap-2 justify-end">
							<button
								...
								class="px-3 py-1.5 text-sm bg-primary-100 hover:bg-primary-100 rounded-md text-primary-700"
							>
								{s('common.cancel') || 'Cancel'}
							</button>
							<button
								onclick={confirmImport}
								class="px-3 py-1.5 text-sm bg-purple-600 hover:bg-purple-700 text-white rounded-md"
							>
	...
	<!-- Reset Button -->
	<div class="pt-4 border-t border-primary-100 relative">
		<button
		...
		{#if showResetConfirm}
			<div
				...
				class="absolute bottom-full mb-2 left-0 z-50 animate-in fade-in slide-in-from-bottom-1"
			>
				<div class="bg-modal-bg rounded-lg shadow-lg border border-primary-100 p-4 w-72">
