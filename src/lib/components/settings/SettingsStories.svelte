	...
	<div class="space-y-4">
		...
		<div class="ps-2">
			...
			{#if preferredSources.list.length === 0}
				<div
					class="text-sm text-primary-600 py-8 text-center border-2 border-dashed border-primary-100 rounded-lg"
				>
					...
					{#each preferredSources.list as domain}
						...
						>
							<div class="flex items-center space-x-3">
								<FaviconImage {domain} alt="{domain} favicon" class="h-5 w-5 rounded-sm" />
							...
							<button
								...
								class="text-primary-400 hover:text-red-500 focus-visible-ring rounded p-1"
								aria-label={s('settings.preferredSources.remove') ||
									`Remove ${domain} from pinned sources`}
							>
								<IconX class="h-4 w-4" />
							</button>
						</div>
