...
{#if isOpen}
	...
	<div
		class="fixed inset-0 bg-black/50 flex {positionClasses[position]} p-0 sm:p-4"
	...
	>
		...
		<div
			...
			class="relative w-full h-full sm:h-auto {sizeClasses[size]} {position === 'bottom'
				? 'rounded-t-2xl'
				: 'sm:rounded-lg'} bg-modal-bg shadow-xl flex flex-col {className}"
			role="dialog"
			aria-modal="true"
		...
		>
			...
			{#if title || showCloseButton}
				<div
					class="flex items-center justify-between border-b border-primary-100 p-4 shrink-0 relative z-10 bg-modal-bg"
				>
					{#if title}
						<h2 id="modal-title" class="text-lg font-semibold text-primary">
							{title}
						</h2>
					{:else}
						<div></div>
					...
					{#if showCloseButton}
						<button
							onclick={onClose}
							class="rounded-full p-2 text-primary-600 hover:bg-primary-50 hover:text-primary-700 focus-visible-ring touch-manipulation min-h-11 min-w-11 flex items-center justify-center"
