...
<div class="relative">
	...
	<div class="relative">
		<svg
			class="absolute start-3 top-1/2 -translate-y-1/2 size-5 text-primary-400"
			fill="none"
			stroke="currentColor"
			viewBox="0 0 24 24"
		>
		...
		<div class="flex items-center flex-wrap gap-2 ps-10 pe-4 py-3">
			...
			{#each filters as filter, index}
				<span
					class="filter-chip inline-flex items-center gap-1 px-2 py-0.5 text-sm font-medium rounded-md select-none bg-purple-100 text-accent-links dark:bg-purple-900"
				>
					...
					<button
						...
						class="ml-1 hover:bg-purple-200 dark:hover:bg-purple-800 rounded p-0.5"
			...
			<div
				...
				class="flex-1 min-w-[200px] text-primary bg-transparent border-0 focus:outline-none focus:ring-0"
	...
	{#if suggestions.length > 0}
		<div
			class="absolute z-50 w-full mt-1 bg-modal-bg border border-primary-100 rounded-md shadow-lg"
		>
			<div bind:this={suggestionsContainer} class="py-1 max-h-60 overflow-y-auto">
				{#each suggestions as suggestion, index}
					<button
						class="w-full px-3 py-2 text-start hover:bg-primary-50 flex items-center justify-between group {index ===
						selectedSuggestionIndex
							? 'bg-primary-50'
							: ''}"
					...
					>
						...
						{#if suggestion.isFilterType}
							<span class="text-xs text-primary-400 opacity-0 group-hover:opacity-100">
								filter
							</span>
						{/if}
					</button>
