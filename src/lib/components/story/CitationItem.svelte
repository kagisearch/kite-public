...
{#if item.isCommon}
	...
	>
		...
		<div class="flex-1">
			<div class="font-medium text-primary-700 {isMobile ? 'mb-2' : 'mb-1'}">
				{storyLocalizer('citation.commonKnowledge.title') || 'Common Knowledge'}
			</div>
			<div class="text-primary-600 {isMobile ? 'leading-relaxed' : 'text-xs leading-relaxed'}">
...
{:else if item.article}
	...
	>
		...
		<div class="flex items-center {spacingClasses} flex-1 min-w-0">
			<FaviconImage
				...
				loading="lazy"
			/>
			<span class="font-medium text-primary-700 truncate leading-none">
				{item.article.domain}
			</span>
		</div>
	</div>
	<div class={marginClasses}>
		<a
			...
			class="text-primary-600 hover:underline {linkClasses} block"
			title={item.article.title}
		>
			{item.article.title}
		</a>
		{#if item.article.date}
			<div class="text-primary-600 {dateClasses}">
