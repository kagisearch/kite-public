...
{#if isOpen}
	<div class="w-48 rounded-lg bg-modal-bg shadow-lg border border-primary-100">
		{#if isSimplifying}
			<div class="py-4 px-4 flex items-center justify-center gap-2">
				...
				<span class="text-sm text-primary-600">Simplifying...</span>
			</div>
		{:else}
			<div class="py-1">
				<div class="px-3 py-2 text-xs font-semibold text-primary-600 uppercase">
					Choose Reading Level
				</div>
				<button
					class="w-full px-4 py-2 text-left text-sm text-primary-700 hover:bg-primary-50 transition-colors"
		...
		{#if error}
			<div class="px-4 py-2 text-xs text-red-600 dark:text-red-400 border-t border-primary-100">
				{error}
			</div>
		{/if}
	</div>
