...
<div class="mb-4 rounded-lg border border-primary-100 bg-primary-25 dark:bg-graphite-800/50">
	...
	<div class="flex w-full items-center justify-between px-4 py-3">
		...
		>
			...
			<span class="text-sm font-medium text-primary">{s('nhl.standings.title')}</span>
				...
				{#each topTeams as team, index}
					<div class="flex items-center justify-between text-xs">
						<div class="flex items-center gap-2">
							...
							{/if}
							<span class="font-medium text-primary">{team.teamId}</span>
						</div>
						<div class="flex items-center gap-3">
							<span class="text-primary-600">{team.wins}-{team.losses}-{team.otLosses}</span>
							<span class="min-w-[2rem] text-right font-bold text-primary"
								>{team.points} {s('nhl.standings.points')}</span
							>
						</div>
					</div>
				{/each}
			...
			{:else}
									...
									>
										<tr>
											<th class="px-3 py-2 text-left font-medium"
												>{s('nhl.standings.header.rank')}</th
											>
											<th class="px-3 py-2 text-left font-medium"
												>{s('nhl.standings.header.team')}</th
											>
											<th class="px-2 py-2 text-center font-medium"
												>{s('nhl.standings.header.gp')}</th
											>
											<th class="px-2 py-2 text-center font-medium"
												>{s('nhl.standings.header.w')}</th
											>
											<th class="px-2 py-2 text-center font-medium"
												>{s('nhl.standings.header.l')}</th
											>
											<th class="px-2 py-2 text-center font-medium"
												>{s('nhl.standings.header.ot')}</th
											>
											<th class="px-2 py-2 text-center font-medium"
												>{s('nhl.standings.header.pts')}</th
											>
											<th class="px-2 py-2 text-center font-medium hidden sm:table-cell"
												>{s('nhl.standings.header.diff')}</th
											>
											<th class="px-2 py-2 text-center font-medium"
												>{s('nhl.standings.header.strk')}</th
											>
										</tr>
									</thead>
									<tbody class="bg-white dark:bg-graphite-800">
										{#each division.teams as team, index}
											...
											>
												...
												<td class="px-2 py-2 text-center text-primary-700">{team.gamesPlayed}</td>
												...
												<td class="px-2 py-2 text-center text-primary-700">{team.otLosses}</td>
												<td class="px-2 py-2 text-center font-bold text-primary">{team.points}</td>
												<td class="px-2 py-2 text-center text-primary-700 hidden sm:table-cell">
													<span
														class={team.goalDifferential > 0
				...
				>
					<div class="flex flex-wrap gap-x-3 gap-y-1">
						<span
							><strong>{s('nhl.standings.header.gp')}:</strong> {s('nhl.standings.legend.gp')}</span
						>
						<span
							><strong>{s('nhl.standings.header.w')}:</strong> {s('nhl.standings.legend.w')}</span
						>
						<span
							><strong>{s('nhl.standings.header.l')}:</strong> {s('nhl.standings.legend.l')}</span
						>
						<span
							><strong>{s('nhl.standings.header.ot')}:</strong> {s('nhl.standings.legend.ot')}</span
						>
						<span
							><strong>{s('nhl.standings.header.pts')}:</strong>
							{s('nhl.standings.legend.pts')}</span
						>
						<span class="hidden sm:inline"
							><strong>{s('nhl.standings.header.diff')}:</strong>
							{s('nhl.standings.legend.diff')}</span
						>
						<span
							><strong>{s('nhl.standings.header.strk')}:</strong>
							{s('nhl.standings.legend.strk')}</span
						>
					</div>
				</div>
			{/if}
