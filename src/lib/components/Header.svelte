...
import AppLogoSvg from './AppLogoSvg.svelte';
import KiteSvg from './KiteSvg.svelte';
...
// Show the orbiting beam around the pill on first paint of a time-travel
// URL (5s passive call-to-action so the user can find the close target),
// then again any time the user hovers the URL banner.
let initialBeamActive = $state(false);
$effect(() => {
	const shouldPulse =
		timeTravelBatch.entrySource === 'url' && !settings.timeTravelBannerDismissed.currentValue;
	if (!shouldPulse) {
		// Banner dismissed mid-pulse — kill the beam immediately instead of
		// letting the 5s timeout finish.
		initialBeamActive = false;
		return;
	}
	initialBeamActive = true;
	const timer = setTimeout(() => {
		initialBeamActive = false;
	}, 5000);
	return () => clearTimeout(timer);
});
const showHintBeam = $derived(timeTravelExitHint.active || initialBeamActive);
...
{#snippet dateSection()}
	{#if isExitingTimeTravel}
		...
		<div class="flex items-center gap-2 text-primary-600">
			<div
				class="animate-spin h-4 w-4 border-2 border-primary-200 border-t-purple-600 rounded-full"
			></div>
			<span class="text-sm">{s('timeTravel.returningToLive') || 'Returning to live...'}</span>
		</div>
	{:else if timeTravel.selectedDate}
		<div
			class="flex items-center gap-2 px-2 py-1 rounded-lg tt-pill {timeTravelBatch.entrySource ===
				'url' && !settings.timeTravelBannerDismissed.currentValue
				? 'bg-purple-100 dark:bg-purple-800/50'
				: 'bg-purple-50 dark:bg-purple-900/30'}"
			class:tt-hint-active={showHintBeam}
		>
			<svg class="tt-beam" aria-hidden="true" preserveAspectRatio="none">
				<rect
					x="1.5"
					y="1.5"
					width="calc(100% - 3px)"
			...
			<svg
				...
				class="h-4 w-4 text-accent-links"
				fill="none"
				viewBox="0 0 24 24"
				stroke="currentColor"
			>
			...
			<div class="text-sm font-medium text-accent-links">
				{dateDisplay}
			</div>
			<button
				onclick={async () => {
			...
			>
				<svg class="size-3 text-accent-links" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path
						stroke-linecap="round"
						stroke-linejoin="round"
						stroke-width="2"
	...
	{:else}
		<div
			class="cursor-pointer text-primary-600 text-base focus-visible-ring rounded px-1 py-1"
	...
	<div class="flex items-center justify-between relative h-full">
		...
		{#if isInTimeTravel}
			...
			<div class="sm:hidden flex items-center h-12">
				...
				{:else}
					<div
						class="flex items-center gap-1.5 bg-purple-50 dark:bg-purple-900/30 px-2 py-1 rounded-lg tt-pill"
						class:tt-hint-active={showHintBeam}
					>
						<svg class="tt-beam" aria-hidden="true" preserveAspectRatio="none">
							<rect
						...
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4 w-4 text-accent-links shrink-0"
							fill="none"
							viewBox="0 0 24 24"
							stroke="currentColor"
						>
						...
						<span class="text-sm font-medium text-accent-links">
						...
						>
							<svg
								class="w-3.5 h-3.5 text-accent-links"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
		...
		{:else}
			...
			<div class="sm:hidden flex items-center h-12 {isInTimeTravel ? '' : 'overflow-hidden'}">
				<div
				...
				>
					...
					<div class="h-12 flex items-center">
						...
						>
							<i role="img" class="logo text-primary z-modal-backdrop isolate" aria-label="{s('app.logo.newsAlt') || 'Kite News'}">
								<AppLogoSvg />
							</i>
						</button>
					</div>
					...
					{#each [1, 2, 3, 4, 5] as index}
						<div class="h-12 flex items-center text-base whitespace-nowrap text-primary-600">
							{#if index === 1}
							...
							{:else if index === 2}
								...
								{:else if nextUpdateText}
									{s('time.nextUpdate.in', { time: nextUpdateText }) ||
							...
							{:else if index === 4}
								{#if totalStoriesRead === 1}
									{s('stats.storyRead', { count: totalStoriesRead.toString() }) ||
										`Story read: ${totalStoriesRead}`}
								{:else}
		...
		{#if experimental.showChaosIndex && chaosIndex && chaosIndex.score > 0 && !isAnimating && !isInTimeTravel && !settingsLock.getManagedOption('hideChaosIndex')}
		...
		<div
			class="hidden sm:flex absolute start-1/2 ltr:-translate-x-1/2 rtl:translate-x-1/2 items-center h-12 {isInTimeTravel
				? ''
				: 'overflow-hidden'}"
		>
			{#if showAnimation && !isInTimeTravel}
				...
				>
					...
					>
						...
						{#each [1, 2, 3, 4, 5] as index}
							<div
								class="h-12 flex items-center justify-center whitespace-nowrap text-primary-600 text-base"
							>
								{#if index === 1}
									{lastUpdatedText || getLastUpdated()}
								{:else if index === 2}
		...
		<div class="ms-auto flex items-center gap-1.5 sm:gap-2">
			...
			{#if experimental.showChaosIndex && chaosIndex && chaosIndex.score > 0 && !settingsLock.getManagedOption('hideChaosIndex')}
			...
			{#if !settingsLock.getManagedOption('hideTimeTravel')}
				<button
					onclick={() => timeTravel.toggle()}
					title={s('header.timeTravel') || 'Time Travel'}
					aria-label={s('header.timeTravel') || 'Time Travel'}
					class="p-1 sm:w-8 sm:h-8 sm:flex sm:items-center sm:justify-center sm:rounded-lg sm:hover:bg-primary-50"
					type="button"
				>
					<IconClock size={24} stroke={1.5} class="text-primary-600" />
				</button>
			{/if}

			{#if !settingsLock.getManagedOption('hideSearch')}
				<button
					...
					type="button"
				>
					<IconSearch size={24} stroke={1.5} class="text-primary-600" />
				</button>
			{/if}

			{#if !settingsLock.getManagedOption('hideFontSize')}
				<button
					...
					type="button"
				>
					<IconTextSize size={24} stroke={1.5} class="text-primary-600" />
				</button>
			{/if}
			...
			<button
				...
				type="button"
			>
				<IconSettings size={24} stroke={1.5} class="text-primary-600" />
			</button>

			<!-- Divider -->
			<div class="h-[25px] flex items-center justify-center">
				<div class="w-px h-full bg-primary-100"></div>
			</div>
...
{#if showFlyingKite}
	...
	>
		<i class="flying-kite text-primary" aria-hidden="true">
			<KiteSvg />
		</i>
	</div>
{/if}
...
<style>
/* Border-beam: an SVG <rect> stroked with a dash that travels around the
     path. Unlike CSS Motion Path (which rotates the element rigidly), the
     dash is drawn ALONG the SVG path, so it naturally bends around the
     rounded corners — no visible "rotation" of the line. pathLength="100"
     normalises the perimeter so dash and offset are easy to reason about. */
.tt-pill {
	position: relative;
}
.tt-beam {
	...
	width: calc(100% + 6px);
	height: calc(100% + 6px);
	color: var(--color-accent-links, #7c3aed);
	opacity: 0;
	pointer-events: none;
...
.tt-beam rect {
	...
	/* Animation runs continuously regardless of hint state. Toggling it
       on/off would snap the dash back to its starting position during the
       opacity fade-out, causing a visible jump. Keeping it running means
       re-hover picks up wherever the dash currently is — no jumps. */
	animation: tt-beam-orbit 3.6s linear infinite;
}
.tt-pill.tt-hint-active .tt-beam {
	opacity: 1;
}
@keyframes tt-beam-orbit {
	from {
		stroke-dashoffset: 0;
	}
	to {
