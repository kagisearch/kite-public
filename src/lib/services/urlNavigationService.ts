 ...
 */
function isBatchId(str: string): boolean {
...
function isValidDataLanguage(lang: string): boolean {
...
function parseUrl(url: URL): ParsedUrl {
  ...
  if (firstSegment === "latest") {
    ...
    params.storyIndex = pathSegments[2]
      ? Number.parseInt(pathSegments[2])
      : null;
  } else {
    // Check if first segment looks like a batch ID
    const looksLikeBatchId = isBatchId(firstSegment);

    if (!looksLikeBatchId) {
      // No batch ID, this is latest batch with category
      params.batchId = null;
      params.categoryId = pathSegments[0];
      params.storyIndex = pathSegments[1]
        ? Number.parseInt(pathSegments[1])
        : null;
    } else {
      // Has batch ID
...
function areUrlsDifferent(url1: string, url2: string): boolean {
  return url1 !== url2;
}
...
function getFullUrl(url: URL): string {
  return url.href;
}
